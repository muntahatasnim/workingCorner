function part_time(type) {
    let dataArray = [];
        firebase.database().ref().child('UserPost').orderByChild('Category').equalTo(type).on("value", function (snapshot) {
    ...
    var i = 0;
    for (i = 0; i < len; i++) {
        document.getElementById("userpost").innerHTML += "<p>" + "<b>Category :</b> " + dataArray[i].Category + "</p>";
